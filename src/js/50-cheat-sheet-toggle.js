...
function toggleClassHidden (selector) {
  ...
  document.querySelectorAll('.sectionbody').forEach((el) => {
    const hidden = el.querySelectorAll('div.sect2.hidden').length
    const sects = el.querySelectorAll('div.sect2').length
    // console.log(el)
    // console.log(el.querySelectorAll('div.sect2.hidden').length)
    // console.log(el.querySelectorAll('div.sect2').length)

    const header = el.closest('div.sect1')
    const id = header.firstElementChild.id
